...
On the other hand, on a Type-II superconductor this transition towards normal conductivity is much smoother. In this case, rather than taking one critical magnetic field $H_c$, two critical field values are studied, $H_{c1}$ that represents the value of $H$ for which $M$ stops strictly behaving as $M = - H$; and $H_{c2}$ that represents the value of $H$ for which $M$ exactly zero. As a consequence of this behavior, magnetic flux is able to partially penetrate the material in the interval between $H_{c1} \text{ and } H_{c2}$, in the form of discrete flux vortices as it is represented in .
